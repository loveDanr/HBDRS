    ...
    public class MyBasePage : System.Web.UI.Page
    {
        ...
        private void ProcessPermission(List<Control> list, List<string> listMenu)
        {
            if (list != null)
            {
                for (int i = 0; i < list.Count; i++)
                {
                    //例如：lkBtnDelete__UserList_Delete
                    string s1 = list[i].ID.Substring(list[i].ID.IndexOf("__") + 2);//获取按钮里页面的名字 UserList_Delete
                    string s2 = "";
                    bool hasPermission = false;//是否拥有权限bool变量
                    int index = 0;
                    if (s1.IndexOf("_") != -1)              // UserList_Delete
                    {
                        s2 = s1.Substring(s1.IndexOf("_") + 1); //Delete
                        s1 = s1.Replace("_" + s2, ".aspx");  //去除_Delete
                    }
                    else
                        s1 += ".aspx";  //组成页面 
