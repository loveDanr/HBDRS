...
namespace Project.Dal
{
    ...
    public class UserInfoDal
    {
        ...
        {
            const string sql = "INSERT INTO UserInfo (UserId,UserName,UserPwd,RoleId,DeptId,[Desc],IsLock) VALUES (@UserId,@UserName,@UserPwd,@RoleId,@DeptId,@Desc,@IsLock)";
        ...
        private SqlParameter[] GetParms(UserInfo model)
        {
            ...
            parms[4].Value = model.DeptId;
            if (model.Desc == null)
                parms[5].Value = DBNull.Value;
            else
                parms[5].Value = model.Desc;
            parms[6].Value = model.IsLock;
            return parms;
        }
        ...
        {
            const string sql = "UPDATE UserInfo SET UserId=@UserId, UserName=@UserName, UserPwd=@UserPwd, RoleId=@RoleId, DeptId = @DeptId,[Desc]=@Desc, IsLock=@IsLock WHERE 1=1  AND UserId=@UserId";
        ...
        /// <summary>
        /// 在dbo.UserInfo表中完整更新用户密码,支持数据库事务
        /// </summary>
        /// <param name="model"></param>
        /// <param name="trans"></param>
        /// <returns></returns>
        public int UpdatePwd(UserInfo model, SqlTransaction trans)
        {
            const string sql = "UPDATE UserInfo SET UserPwd=@UserPwd  WHERE 1=1  AND UserId=@UserId";

            SqlParameter[] parms ={new SqlParameter("@UserId", SqlDbType.VarChar,6),			
				                   new SqlParameter("@UserPwd", SqlDbType.VarChar, 50)
                                  };
            // GetParms(model);
            parms[0].Value = model.UserId;
        ...
        public int Delete(string id, SqlTransaction trans)
        {
        ...
        public UserInfo GetModel(string id)
        {
        ...
        private UserInfo LoadModel(IDataReader dr)
        {
            ...
            model.DeptId = dr["DeptId"].ToString();
        ...
        public UserInfo GetModel(string userId, string pwd)
        {
            UserInfo model = null;
            const string sql = "select * from UserInfo where UserId=@UserID and UserPwd=@UserPwd";
            SqlParameter[] parms = {
