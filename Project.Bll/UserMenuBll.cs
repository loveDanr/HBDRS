...
namespace Project.Bll
{
	...
	public class UserMenuBll
	{
        ...
        public  string GetHtmlMenu(string roleId)
        {
            List<UserMenu> allList = GetRoleMenuList(roleId);
            ...
            if (allList != null)
            {
                ...
                for (int i = 0; i < parentList.Count; i++)
                {
                    ...
                    sb.AppendFormat("<div class=\"ti\" onclick=\"sh({0},this)\"><a href='javascript:' >{1}</a></div>", parentList[i].MenuId, parentList[i].MenuName);
                    ...
                    {
                        sb.AppendFormat("<li><a href=\"{0}\" class=\"lfmenuA\"  onclick=\"sha(this)\" target=\"sysMain\">{1}</a></li>", Common.CommonFun.ResolveUrl("~/" + childList[k].MenuAddress), childList[k].MenuName);
                    }
                    sb.AppendFormat("</ul>");
                }
                sb.AppendFormat("</div>");
